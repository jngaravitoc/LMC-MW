...
#vz_ic = 0
Mbulge = 1E10
Mhalo = 1E12#float(sys.argv[1])
Mdisk = 6.5E10 #float(sys.argv[2])
Msat = float(sys.argv[1])
Rvir = 261.0 #float(sys.argv[4])  
rs = 26.47 * units.kpc #float(sys.argv[5]) * units.kpc
rlmc = float(sys.argv[2]) # units.kpc
mw = "free" #sys.argv[7]
...
def acceleration_mw(x, y, z, M_sat):
    ...
    #rlmc = rlmc * units.kpc
    #if (r.value<=Rvir):
    Ax =  a_plummer(rlmc, x.value, y.value, z.value, M_sat.value)[0]        
    Ay =  a_plummer(rlmc, x.value, y.value, z.value, M_sat.value)[1]
    Az =  a_plummer(rlmc, x.value, y.value, z.value, M_sat.value)[2]
    Ax = Ax.to(units.kpc / units.Gyr**2)
    Ay = Ay.to(units.kpc / units.Gyr**2)
    Az = Az.to(units.kpc / units.Gyr**2)
...
print "# t, x(kpc), y(kpc), z(kpc), xmw(kpc/Gyr), ymw(kpc/Gyr), zmw(kpc/Gyr), vxmw(Kpc/Gyr), vymw(Kpc/Gyr), vzmw(Kpc/Gyr)"

for i in range(len(xx)):
	print t[i], xx[i], yy[i], zz[i], xx_mw[i], yy_mw[i], zz_mw[i], vx[i], vy[i], vz[i],  vx_mw[i], vy_mw[i], vz_mw[i] 
